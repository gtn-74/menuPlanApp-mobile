...
const menu = (date: string): MenuItem => ({
  id: `m-${date}`, date, name: '料理', budget: 0, ingredients: [], photos: [],
  userId: 'u', familyGroupId: 'f', createdAt: '',
});
const budget = (date: string): BudgetItem => ({
  id: `b-${date}`, date, category: 'food', amount: -100, description: '',
  userId: 'u', familyGroupId: 'f', createdAt: '',
});
const event = (date: string, type: Event['type']): Event => ({
  id: `e-${date}-${type}`, date, title: '予定', time: '10:00', type,
  userId: 'u', createdAt: '',
});
const todo = (date: string): TodoItem => ({
  id: `t-${date}`, date, title: 'やること', completed: false, priority: 'high',
  userId: 'u', createdAt: '',
});

const ALL_ON: CategoryFlags = {
  showMenu: true, showBudget: true, showPersonalEvents: true,
  showFamilyEvents: true, showTodo: true,
};
const ALL_OFF: CategoryFlags = {
  showMenu: false, showBudget: false, showPersonalEvents: false,
  showFamilyEvents: false, showTodo: false,
};
...
describe('buildMarkedDates', () => {
  it('献立だけの日には menu ドットが1つ付く', () => {
    const marked = buildMarkedDates({
      ...empty, menus: [menu('2026-01-10')], flags: ALL_ON, selectedDate: '2026-01-01',
    });
    expect(marked['2026-01-10'].dots).toEqual([
      { key: 'menu', color: colors.dots.menu },
    ]);
  });
  ...
  it('同じ日に複数カテゴリがあるとドットが順番に積まれる', () => {
    const d = '2026-01-10';
    const marked = buildMarkedDates({
      menus: [menu(d)], budgets: [budget(d)],
      events: [event(d, 'personal'), event(d, 'family')], todos: [todo(d)],
      flags: ALL_ON, selectedDate: '2026-01-01',
    });
  ...
  it('フラグが OFF のカテゴリはドットにならない', () => {
    const d = '2026-01-10';
    const marked = buildMarkedDates({
      menus: [menu(d)], budgets: [budget(d)], events: [], todos: [],
  ...
  it('personal/family は各フラグで出し分けられる', () => {
    ...
    const onlyFamily = buildMarkedDates({
      ...empty, events: [event(d, 'personal'), event(d, 'family')],
      flags: { ...ALL_OFF, showFamilyEvents: true }, selectedDate: '2026-01-01',
    });
  ...
  it('選択日にドットがある場合は selected:true が付く', () => {
    const d = '2026-01-10';
    const marked = buildMarkedDates({
      ...empty, menus: [menu(d)], flags: ALL_ON, selectedDate: d,
    });
